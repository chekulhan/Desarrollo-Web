...
En express, tienes los siguientes coches. Punlicarlos a travñes de un api/coches, y accederlos desde un componente de ReactJS.
...
const ListaCoches = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/coches')  // Make a GET request to the API
      .then((respuesta) => {
        if (!respuesta.ok) {
